...
class FEMPreProcessor:
    ...
    def add_boundary_condition(self, node_id, ux=0, uy=0, uz=0, rx=0, ry=0, rz=0):
        """Add boundary condition (0=free, 1=fixed) for all 6 DOF"""
    ...
    def add_load(self, node_id, fx=0.0, fy=0.0, fz=0.0, mx=0.0, my=0.0, mz=0.0):
        """Add load and moments at node (forces in N, moments in N·m)"""
    ...
    def set_default_bcs_and_loads(self):
        ...
        # COMPLEX GEOMETRY: Original crane structure
        # Fixed supports at new nodes (30, 31, 32) - all 6 DOF fixed
    ...
    def export_to_file(self, filename='data/structure.dat'):
        ...
        with open(filename, 'w') as f:
            # Header
            f.write("# FEM Structure Data File - BEAM ELEMENTS\n")
            ...
            f.write("PARAMETERS\n")
            f.write(f"{E} {props['A']:.10e} {props['Iz']:.10e} {props['Iy']:.10e} {props['J']:.10e} {G:.10e} {self.diameter:.10e}\n")
            f.write("# E(Pa) A(m²) Iz(m⁴) Iy(m⁴) J(m⁴) G(Pa) diameter(m)\n\n")
            
            print(f"\n  Beam Cross-Section Properties (Circular):")
            print(f"    Diameter: {self.diameter*1000:.2f} mm")
            print(f"    Area: {props['A']*1e6:.4f} mm²")
            print(f"    Iz = Iy: {props['Iz']*1e12:.4f} mm⁴")
            print(f"    J (torsion): {props['J']*1e12:.4f} mm⁴")
            print(f"    G (shear modulus): {G/1e9:.1f} GPa\n")
            
            # Materials
            f.write("MATERIALS\n")
            for mat_name, props_mat in self.materials.items():
                f.write(f"{mat_name} {props_mat['E']:.6e} {props_mat['nu']:.6f}\n")
            f.write("\n")
            ...
            # Loads
            f.write("LOADS\n")
            f.write("# Node_ID Fx Fy Fz Mx My Mz (Forces in N, Moments in N·m)\n")
            for node_id, load in self.loads.items():
                f.write(f"{node_id} {load['Fx']:.6f} {load['Fy']:.6f} {load['Fz']:.6f} {load['Mx']:.6f} {load['My']:.6f} {load['Mz']:.6f}\n")
            f.write("\n")
